...
def render_gridmap(screen, xw, yw, scale, gridmap):
  #screen.fill("white")

  for x in range(0, xw):
    for y in range(0, yw):
      value = 1.0 - 1 / math.exp(1 + gridmap[xw * x + y])
      shade = min(255, max(0, int(value * 255)))
      color = pygame.Color(shade, shade, shade)

      rect = pygame.Rect(x * scale, y * scale, scale, scale)
      pygame.Surface.fill(screen, color=color, rect=rect)
...
def lidar_callback(screen, grid):
    size = pygame.display.get_window_size()
    render_gridmap(screen, grid.width, grid.height, size[0] / grid.width, grid)


def generate_lidar_bp(arg, world, blueprint_library, delta):
    """Generates a CARLA blueprint based on the script parameters"""
    lidar_bp = blueprint_library.find('sensor.lidar.ray_cast_ocl')
    return lidar_bp
...
def main(arg):
    ...
    try:
        ...
        lidar.listen(lambda data: lidar_callback(screen, data))
